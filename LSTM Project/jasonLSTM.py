...
for test_site in site_ids:
    ...
    df_train = pd.concat([df_majority, df_minority_oversampled]) #comment this out to stop minority over sampling

    # Build sequences for time series: training set
    X_train_ts, y_train = [], []
    ...
    # Build sequences for time series: test set 
    X_test_ts, y_test = [], []
    ...
    #dense1 = Dense(32, activation='swish')(concatenated)  # Changed from 'relu' to swish
    output = Dense(1, activation='sigmoid')(dense1)

    model = Model(inputs=[input_ts, input_cross], outputs=output)

    #model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
    model.compile(optimizer='adam', loss=focal_loss(), metrics=['accuracy']) #focal loss 
    ...
    y_pred = (y_pred_probs > 0.5).astype(int) #normal threshhold 0.5
