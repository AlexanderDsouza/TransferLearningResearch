...
def train_with_new_encoder_and_prediction_head_cv(
    ...
    return encoder, ep1_prediction_head
...
def find_wrong_predictions(
    source_data_ep2, latent_dim, ep1_prediction_head,misclassification_counter,confidence_tracker,risk_tracker,pred_tracker,sids, epochs=20, batch_size=8, n_splits=10
):
    test_losses = []
    test_accuracies = []
    aucs = []
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        for idx, prob, true_label in zip(val_idx, y_pred_probs.flatten(), y_val):
            sid = sids[idx]  # Get the actual SID
            confidence = 2 * max(prob, 1 - prob) - 1  # Now in [0, 1]
            risk = 1 - confidence  # Also in [0, 1]
            
            pred_tracker[sid].append(prob)
            confidence_tracker[sid].append(confidence)
            risk_tracker[sid].append(risk)
        ...
        incorrect_sids = sids[incorrect_val_indices]  # get actual SIDs of misclassified points

        print("Incorrect SIDs this fold:", incorrect_sids.tolist())

        # Update the counter with the indices of misclassified points
        misclassification_counter.update(incorrect_sids)
...
def domain_adaptation_simple(file_path):
    ...
    ep2_data_sid = ep2_data_sid.drop_duplicates(subset="SID", keep="first")
    ...
    pred_tracker = defaultdict(list)

    sids = ep2_data_sid["SID"].values  # Index-aligned with your X/y
    misclassification_counter = Counter({sid: 0 for sid in sids})


    total_cv_runs = 100 # total outer loops


    for i in range(total_cv_runs):
        print(f"Run {i+1}/{total_cv_runs}")
        encoder_ep2, prediction_head, auc, acc = find_wrong_predictions(
    ...
    sid_misclass_counter = defaultdict(int)
    sid_pred_tracker = defaultdict(list)



    # Aggregate predicted probabilities per SID
    for sid, preds in pred_tracker.items():
        sid_pred_tracker[sid].extend(preds)

    # Compute average predicted probability per SID
    sid_pred_avg = {
        sid: np.mean(preds) if len(preds) > 0 else np.nan
        for sid, preds in sid_pred_tracker.items()
    }

    # Aggregate misclassifications per SID
    for sid, count in misclassification_counter.items():
        print(sid)
        sid_misclass_counter[sid] += count

    # Build DataFrames
    df_sid_pred = pd.DataFrame.from_dict(
        sid_pred_avg, orient='index', columns=['avg_pred_prob']
    ).reset_index().rename(columns={'index': 'SID'})

    df_sid_misclass = pd.DataFrame.from_dict(
        sid_misclass_counter, orient='index', columns=['misclass_count']
    ).reset_index().rename(columns={'index': 'SID'})
    ...
    max_val_appearances = total_cv_runs
    df_sid_misclass['misclass_percent'] = df_sid_misclass['misclass_count'] / max_val_appearances
    df_sid_misclass = df_sid_misclass[['SID', 'misclass_percent']]

    # Merge and export
    df_sid_summary = pd.merge(df_sid_misclass, df_sid_pred, on='SID', how='left')
    df_sid_summary.to_csv("sid_misclass_summary2.csv", index=False)

    all_indices = list(range(len(ep2_data)))

    # misclassification plot 
    misclass_counts = [misclassification_counter.get(i, 0) for i in sids]
    ...
    bars = ax.bar(sorted_indices, conf_error_values, color=colors, edgecolor='black')
    sm = plt.cm.ScalarMappable(cmap='Reds', norm=norm)
    sm.set_array([])  # To avoid warning about empty data
    fig.colorbar(sm, ax=ax, label='Times Misclassified')
    ax.set_xlabel('Data Point Index')
    ax.set_ylabel('Average Confidence 2 * max(prob, 1 - prob) - 1')
    ...
    ax.set_ylim(0, 1)  # Keep confidence error between 0 and 1
    plt.savefig(desktop_path)
    plt.tight_layout()
    plt.xticks(rotation=45)  # or rotation=90 for vertical labels
    plt.show()
    ...
    print("\n========== FINAL AVERAGES ==========")
    print(f"Average AUC over 100 runs: {overall_avg_auc}")
    print(f"Average Accuracy over 100 runs: {overall_avg_accuracy}")
...
#just getting averages
    for i in range(10):
        print(f"\n======= Run {i+1}/10 =======")
        encoder_ep2, prediction_head, auc, acc = train_with_new_encoder_and_prediction_head_cv_tsne(
            source_data_ep2=ep2_data,
            latent_dim=64,
            ep1_prediction_head=prediction_head,
            epochs=20,
