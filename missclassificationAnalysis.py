...
def find_wrong_predictions(
    source_data_ep2, latent_dim, ep1_prediction_head,misclassification_counter,confidence_tracker,risk_tracker,pred_tracker, epochs=20, batch_size=8, n_splits=10
):
    test_losses = []
    test_accuracies = []
    aucs = []
...
def domain_adaptation_simple(file_path):
    ...
    for idx, preds in pred_tracker.items():
        sid = sids[idx]
    ...
    for idx, count in misclassification_counter.items():
        sid = sids[idx]
        sid_misclass_counter[sid] += count

    df_sid_pred = pd.DataFrame.from_dict(sid_pred_avg, orient='index', columns=['avg_pred_prob']).reset_index().rename(columns={'index': 'SID'})
    df_sid_misclass = pd.DataFrame.from_dict(sid_misclass_counter, orient='index', columns=['misclass_count'])
    # Reset index to make SID a column


    df_sid_misclass = df_sid_misclass.reset_index().rename(columns={'index': 'SID'})

    max_val_appearances = total_cv_runs  # each SID shows up in val set once per run
    ...
    plt.ylim(0, max(misclass_counts) + buffer)
