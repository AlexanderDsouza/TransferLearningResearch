...
def domain_adaptation(file_path):
    ...
    features_df = features_df.drop(columns=['Diagnosis','BL_MedsType'])

    misclassification_df = pd.read_excel("sid_misclass_summary2.csv")
    misclassification_df = misclassification_df.drop(columns=['avg_pred_prob','confidence'])

    cueB_minus_cueA_EP2 = cueB_minus_cueA_EP2.merge(features_df,on='SID',how = 'inner')
